...
const ViewReports = (props) => {
  ...
  const columns = [
    ...
    { field: "open", headerName: "Status",  width: 200 },
  ...
  const DataSet = [
    {
      ...
      data: resultJobs
        .filter((job, key) => selectionModel.includes(key + 1))
        .map((job) => [
          { value: job.companyName, style: { font: { sz: "14" } } },
          {
            value: job.noOfStudentsPlaced || "N/A",
            style: { font: { sz: "14" } },
          },
  ...
  const getJobs = async (course, company, year) => {
    ...
    res = await axios.get(
      encodeURI(`/job/getjobs/${course}/${company}/${newyear}`)
    );
    ...
    if (res !== "") {
      ...
      jobs.map((job, key) => {
        const jobNotExpired =
          ...
          new Date(job.dateOfExpiry) > new Date();
        const {
          companyName,
          location,
          salaryPackage,
        ...
        resJobs.push({
          ...
          id: key + 1,
          _id,
          open: isOpen && jobNotExpired ? "Open" : "Closed",
          jobType,
          noOfStudentsPlaced,
        });
      });
  ...
  const handleSubmit = async () => {
    ...
    if (count === 1) {
      ...
      window.location.assign(encodeURI(`${url}/job/download/zip`));
      // window.open(encodeURI(`${url}/job/downloadZip/zip`));
    }
    setLoading(false);
    setDownloadCount(0);
  };
  const labels = resultJobs
    .filter((job, key) => selectionModel.includes(key + 1))
    .map((job) => job.companyName);
  const data1 = resultJobs
    .filter((job, key) => selectionModel.includes(key + 1))
    .map((job) => job.noOfStudentsPlaced || 10);
  const data = {
    labels: labels,
    datasets: [
      {
            ...
            {selectionModel.length >= 1 && (
              <Grid item xs={12} sm={2}>
                <ExcelFile
                  ...
                  element={
                    <Tooltip
                      title="Download placed students count?"
                      placement="top"
                    >
                  ...
                  <Grid item xs={12} sm={3}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          value="firstName"
                          checked={state.firstName.checked}
                  ...
                  <Grid item xs={12} sm={3}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          value="lastName"
                          checked={state.lastName.checked}
                          onChange={(e) => handleChange(e)}
                          name="lastName"
                        />
                      }
                  ...
                  <Grid item xs={12} sm={3}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          value="branchName"
                          checked={state.branchName.checked}
                          onChange={(e) => handleChange(e)}
                          name="branchName"
                        />
                      }
                  ...
                  <Grid item xs={6} sm={3}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          value="tenthCgpa"
                          checked={state.tenthCgpa.checked}
                          onChange={(e) => handleChange(e)}
                          name="tenthCgpa"
                        />
                      }
                  ...
                  <Grid item xs={6} sm={3}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          value="twelfthCgpa"
                          checked={state.twelfthCgpa.checked}
                          onChange={(e) => handleChange(e)}
                          name="twelfthCgpa"
                        />
                      }
                  ...
                  <Grid item xs={6} sm={3}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          value="ugPercentage"
                          checked={state.ugPercentage.checked}
                          onChange={(e) => handleChange(e)}
                          name="ugPercentage"
                        />
                      }
                  ...
                  <Grid item xs={6} sm={3}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          value="pgPercentage"
                          checked={state.pgPercentage.checked}
                          onChange={(e) => handleChange(e)}
                          name="pgPercentage"
                        />
                      }
                  ...
                  <Grid item xs={6} sm={3}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          value="offerLetter"
                          checked={state.offerLetter.checked}
                          onChange={(e) => handleChange(e)}
                          name="offerLetter"
                        />
                      }
                ...
                <>
                  ...
                  >
                    <LinearProgressWithLabel
                      value={Math.round(
                        downloadCount * (100 / selectionModel.length)
                      )}
